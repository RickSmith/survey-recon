...
from corridor_screen import control, output, row_maps
...
def a_document(status="complete", parcels=(), control=None, row_maps=None):
...
class TestShape(unittest.TestCase):
    ...
    def test_the_row_map_block_is_carried_through_as_it_was_built(self):
        """``row_maps.block`` owns the shape. ``output`` only carries it."""
        block = row_maps.block([], without_shape=0)
        document = a_document(row_maps=block)
        self.assertEqual(document["row_maps"], block)

    def test_a_run_with_no_row_map_block_still_says_what_happened(self):
        """The fallback is for a caller that never got that far, not a blank."""
        document = a_document()
        self.assertEqual(document["row_maps"]["status"], "not-screened")
        self.assertTrue(document["row_maps"]["detail"])
