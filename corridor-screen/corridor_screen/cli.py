...
def _corridor_extent(alignment, half_width_ft):
    """The box asked about by the two steps that are not joined to a parcel.

    A box around the corridor itself, not around the parcels. Neither the NGS
    marks nor the ROW map sheets hang off a parcel -- a mark is in the corridor
    or it is not, whoever owns the ground, and a ROW sheet draws the strip of
    ground the parcels sit beside. So the corridor is what both questions are
    about, and ``_parcel_extent`` is the one the flag services use instead.
