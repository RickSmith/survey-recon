...
- ~~TCP(S-1)-08A. The host blocks automated fetch, so it must be pulled by hand. It is the crew-time document, and the traffic-control cost cliff is real: a twenty-minute shot on a 55 mph highway turns a two-person crew into a crew plus a shadow truck with an attenuator.~~
    ...
    **Settled 2026-09-13 under [#11](https://github.com/RickSmith/survey-recon/issues/11), and the second sentence was wrong.** No posted speed anywhere in the six-sheet family triggers a shadow truck; the duration line is one hour, and where the work is decides the cost. Struck rather than deleted because how it got in is the lesson. Full account: `project-sh16/manual-pulls/tcp-s-family.md`. The same wording is still live in `docs/txdot-research.md`, which is [#70](https://github.com/RickSmith/survey-recon/issues/70)'s to fix, not this specification's.
