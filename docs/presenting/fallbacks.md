...
| 0:30–0:46 | Act I — the grilling | not recorded — #75 | The live grilling, and the work orders it writes onto the projector. Read the note below before the day |
...
| 0:57–1:18 | Act II — find the control | `docs/scenarios/sh16/capture-note.md` · `project-sh16/screening.json` | Eleven NGS marks, every one `MARK NOT FOUND`; two distinct TxDOT monuments; 69 ROW sheets reaching the corridor. The capture note reads the findings out in the order you need them |
...
| 1:54–2:00 | Accountability, Monday morning, the live issue | `.github/ISSUE_TEMPLATE/introduce-yourself.yml` | The form you would have opened, on screen. It cannot collect a single reply without GitHub — point at the QR code instead and ask the room to do it from their seats |
...
Four blocks have nothing to reach for. In three of them the thing that would be
recorded does not exist yet, so there is nothing an audit can do but name it:
the deck (#12, then #31), the money slide (#32) and the Hermes demo (#34).
...
- every file named above is committed, and is not empty
- every command named above **runs with the network taken away** — removed at
  the socket, not politely asked to stay home
...
which asked whether the set was complete. It was not, and the four gaps above
are the answer.
