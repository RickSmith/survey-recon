...
| 0:57–1:18 | The one genuinely live call | `docs/scenarios/sh16/capture-note.md` | The NGS cross-check, last seen agreeing on 2026-09-19: five marks in both, five conditions agreeing. **It can fail two ways.** NGS answers `HTTP 200` with an empty list, or the call does not go through at all. Both happened on 2026-09-14, and the second was a 403 from NGS rather than the room. Read the error line before you say whose end it is: a 403 is theirs, a timeout is probably ours. Losing it costs the live moment and nothing else, because the screening run makes no network calls at all. Say that out loud and move on. [The dry run](dry-run.md) has the words for each |
