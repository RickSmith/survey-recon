...
| **The allowlist** | `corridor-screen/tests/plain_language_allowlist.txt` — the pages that do not meet the standard yet, and are skipped until they do. A debt, not an exemption: each child of #133 deletes its own lines, and when the file is empty it gets deleted. Its length is how much of #133 is left |
| **Exempt** | Held out of the plain-language standard for good, which is the opposite of on the allowlist. `docs/corridor-screen/spec.md` and `docs/txdot-research.md` are the agent's own dated output and rewriting them would make the stage demo a lie. See [ADR 0003](docs/adr/0003-produced-artifacts-stay-as-produced.md) |
