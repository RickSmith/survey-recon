...
did, and checking rather than assuming is what found that out.
...
| **Job page** | What a screening run is handed over as: one file the surveyor makes on their own laptop and sends to a principal, carrying the flagged parcels, why each one is flagged, and the run's [honesty block](#corridor-screening-vocabulary). It opens in a web browser with the Wi-Fi off, and printing it is the PDF. Nearest thing in survey work is an exhibit — drawn for the person receiving it rather than for the file it came out of. **Not a "dashboard,"** which is a software word a principal would not use. **Nothing builds one yet.** The word is written down here because every work order under [issue #188](https://github.com/RickSmith/survey-recon/issues/188) uses it |
